...
dvbpsi_psi_section_t* dvbpsi_GenSDTSections(dvbpsi_sdt_t* p_sdt)
{
	...
	p_current->i_table_id = 0x042; // 0x46 is still OK
	p_current->b_syntax_indicator = 1;
	p_current->b_private_indicator = 0;
	p_current->i_length = 12; // section length = the remaining header + CRC32                  
	...
	while(p_service != NULL)
	{
		...
		&& ((p_service_start - p_current->p_data) + i_service_length <= 1020))
		{
		...
		/* p_service_start is where the service begins */
		p_service_start[0] = p_service->i_service_id >> 8;
		p_service_start[1] = p_service->i_service_id;
		
		p_service_start[2] = (p_service->b_eit_schedule ? 0x02 : 0) | (p_service->b_eit_present ? 0x01 : 0);
		p_service_start[2] |= 0xFC ; // 1111 1100
			
		p_service_start[3] = p_service->i_running_status<<5;
		p_service_start[3] |= (p_service->b_free_ca ? 0x10: 0);
